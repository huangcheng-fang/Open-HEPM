...
function Model=Define_particle_set(Model,setID,varargin)
%==========================Check input=====================================
rangeX=[-inf,inf];rangeY=[-inf,inf];rangeZ=[-inf,inf];rangePart=[];rangeR=[];R=eye(3);
...
if ~isempty(rangePart)
    loc=find(ismember(Mesh.pinpart,rangePart));
else
    loc=(1:size(Mesh.pinpart,1)).';
end
%--------------------------------------------------------------------------
coor=Mesh.particles(loc,:);
...
%--------------------------------------------------------------------------
Model.Set.particle_set{setID,1}=loc;
end