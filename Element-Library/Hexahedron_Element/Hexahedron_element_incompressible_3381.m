...
function Hex=Hexahedron_element_incompressible_3381(elements,nodes,IntegralOder)
...
for ei=1:1:size(elements,1)
    ...
    %----------------------------------------------------------------------
    if sum(detJacs,'all')<0;error('negative area');end
    %----------------------------------------------------------------------
    Bv=mean(Bs_element(1:3,:,:),1);
    Bv_mean=sum(pagemtimes(Bv,detJacs),3)/sum(detJacs,3);
    Bs_incompressible=Bs_element;
    Bs_incompressible(1:3,:,:)=Bs_incompressible(1:3,:,:)-repmat(Bv,3,1,1)+repmat(Bv_mean,3,1,IPNUM);
    %----------------------------------------------------------------------
    Hex.DetJac(:,:,:,ei)=detJacs;
    Hex.B_matrix(:,:,:,ei)=Bs_incompressible;
