...
function [Dct,stress,pstrain,histPara]=Modified_Cam_Clay3D_new(elasPara,plasPara,hardPara,histPara,stress,pstrain,dstrain)
kappa=elasPara(1);niu=elasPara(2);
lenda=plasPara(1);M=plasPara(2);
...
while true
    ...
    % Hardening
    pc_corrected=pc0*exp(-theta*dLamda*dGdSigv);
    % Calculate residuals
    ...
    if abs(F)<refF*tol&&norm(sigma_res)<tol*norm(dsigma)&&abs(pc_res)<tol*abs(pc); break;end
...
function [dsigma,K,D_elastic]=Porous_elastic0(kappa,niu,e,p,destrain,dstrainv)
...
% function dF=Get_yield_derivative()
% 
% end
...
% if J2~=0
%     Lode=real(asin(complex(-1.5*sqrt(3)*J3/sqrt(J2)^3))/3);
% else
%     Lode=0;J2=1e-32;
% end
sbar=sqrt(3*J2);
...
% end
% 
% function H_arg=Get_hardening_argument(pstrain)
% H_arg=sum(pstrain(1:3));
% end

function [pc,H]=Hardening(theta,pc,dpev)
pc=pc*exp(theta*-dpev);
H=-theta*pc;
end