...
function DC=Get_displacement_constraint(Boundary,Node_set,nodes,DOF)
%--------------------------------------------------------------------------
...
for bi=1:1:numel(Boundary)
    ...
    direct=Boundary(bi).direction(:);
    value=value_or_expr(Boundary(bi).value,Boundary(bi).expression,nodes(nid,:));
    dof=DOF(direct,nid)';
    container(bi).c1=[dof(:),value(:)];
end
DC=Merge_container(container,1);
if isempty(DC);DC=zeros(0,2);end
[~,loc]=unique(DC(:,1),'last');
DC=DC(loc,:);
end
